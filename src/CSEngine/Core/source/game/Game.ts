...
// src/workers/orchestrator.ts instead, which runs the engine split across
...
// that render.worker.ts's init() now ports piece by piece; it is not part of
// the build's entry graph anymore and can be deleted once nothing in
// render.worker.ts/gamelogic.worker.ts still needs to be cross-checked
// against it.
